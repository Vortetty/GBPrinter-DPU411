...
public:
	...
	HalfStepper(d_word, byte, byte, SteppingMode = SteppingMode::HALF);
	HalfStepper(d_word, byte, byte, byte, byte, SteppingMode = SteppingMode::HALF,
	...
	void SetPosition(d_word);
	d_word GetPosition() const;
	...
	void Step(long = 1);
	void StepForward(d_word = 1);
	void StepBackward(d_word = 1);
	void StepTo(d_word);
...
protected:
	...
	static const d_word _DELAY_MS_DEFAULT_SPEED = 5;
	...
	_word _SpeedRPMs = 0;
	d_word _DelayMS = _DELAY_MS_DEFAULT_SPEED;

	// STATE TRACKING
	d_word _Position = 0;
	d_word _LastStepMS = 0;
